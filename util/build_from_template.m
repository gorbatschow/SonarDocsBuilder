function status = build_from_template(src_file, root, dst_path)
    ...
    try
        ...
        error(['@MDT_ERROR: File not found ' src_file]);
    end
    ...
    while true
        ...
        if strcmpi(token.action, '@mdt_print_equation')
            status = print_equation(status, token);
        elseif strcmpi(token.action, '@mdt_print_equation_boxed')
            status = print_equation(status, token, 'boxed');
        elseif strcmpi(token.action, '@mdt_print_code')
            status = print_code(status, token);
        elseif strcmpi(token.action, '@mdt_print_markdown')
            status = print_markdown(status, token);
        elseif strcmpi(token.action, '@mdt_eval_code')
            status = eval_code(status, token);
        elseif strcmpi(token.action, '@mdt_print_reference')
            status = print_reference(status, token);
        else
            status = replace_text(status, token, ...
                '@MDT_ERROR: Bad command'); 
        end
    end
...
function token_idx = find_mdt_token(tokens)
    for i = 1:numel(tokens)
        ...
        if strcmpi(tokens(i).action(1:4), '@mdt')
            token_idx = i;
            return
        end
    end
...
function status = print_equation(status, token, style)
    ...
    if read_flag < 0
        status = replace_text(status, token, ...
            ['@MDT_ERROR: File not found ' fname]); 
    else
...
function status = eval_code(status, token)
    ...
    elseif strcmpi(result.data_type, 'table')
        ...
        status = replace_text(status, token, ...
            ['@MDT_ERROR: Bad eval result ' script]);
    end
end
% -------------------------------------------------------------------------
